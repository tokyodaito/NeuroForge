...
func m6UsageSnapshot() *budget.AggregatedSummary {
	...
	records := []budget.UsageRecord{
		{Engine: "alpha", Model: "alpha-pro", Tier: "STANDARD",
			...
			ProviderType: budget.ProviderCoding, At: now.Add(-2 * time.Hour)},
		{Engine: "alpha", Model: "alpha-lite", Tier: "SMALL",
			...
			ProviderType: budget.ProviderCoding, At: now.Add(-1 * time.Hour)},
		{Engine: "beta", Model: "beta-frontier", Tier: "FRONTIER",
			...
			ProviderType: budget.ProviderCoding, At: now.Add(-30 * time.Minute)},
	}
	for _, r := range records {
		bc.Record(r)
	}
	s := bc.Summary(startOfDayUTC(now), now)
	return &s
}
